    ...
    public function execute($tpl, $fid, $tplPath, $cplPath, $cplHead)
    {
        ...
        if ($this->plugin_dir) {
            ...
            while ($plugin_file = $d->read()) {
                ...
                if ('object' === $match[1]) {
                    if (\in_array($plugin, $this->obj_plugins, true)) {
...
break;
    ...
    {
        return "self::render('" . \trim(\substr($statement, 1)) . "')";
    }
        ...
        foreach ($token as $key => &$current) {
            ...
            switch ($current['name']) {
                ...
                case 'assoc_array':
                    ...
                    break;
                case 'sam':
                    if (false === \in_array($prev['name'], ['string', 'number'], true)) {
                        if (true === $this->debug) {
                            \pr($xpr, $current, __LINE__);
                        }
                ...
                case 'static_object_sign':
                    ...
                    break;
                case 'operator':
                    if (false === \in_array($prev['name'], ['', 'right_parenthesis', 'right_bracket', 'number', 'string', 'string_number', 'quote', 'assign', 'comma', 'sam', 'sam2'], true)) {
                        return false;
                ...
                case 'left_bracket':
                    ...
                    if (false === \in_array($prev['name'], ['', 'assign', 'left_bracket', 'right_bracket', 'comma', 'left_parenthesis', 'string', 'string_number'], true)) {
                        throw new Compiler\Exception(__LINE__ . ' parse error : file ' . $this->filename . ' line ' . $line . ' ' . $prev['org'] . $current['org']);
                    }
                    $xpr .= $current['value'];
